...
class yahboomcar_driver(Node):
    def __init__(self, name):
        ...
        # 新增：订阅turn_command话题
        self.sub_turn = self.create_subscription(String, "turn_command", self.turn_callback, 10)
        self.current_turn_command = 0  # 默认转向指令为0
    ...
    def turn_callback(self, msg):
        ...
        try:
            cmd = int(msg.data)
            if 0 <= cmd <= 5:  # 验证指令范围0-5
                self.current_turn_command = cmd
                self.get_logger().info(f"收到转向指令: {cmd}")
            else:
                self.get_logger().warn(f"无效转向指令: {cmd}")
                self.current_turn_command = 0
        except ValueError:
            self.get_logger().warn(f"非数字转向指令: {msg.data}")
            self.current_turn_command = 0
    ...
    def cmd_vel_callback(self, msg):
        ...
        # 获取基础运动参数
        vx = msg.linear.x * self.xlinear_limit
        vy = msg.linear.y * self.ylinear_limit
        angular = msg.angular.z * self.angular_limit
        ...
        self.get_logger().debug(
            f"运动控制: vx={vx:.2f}, vy={vy:.2f}, angular={angular:.2f}, turn={self.current_turn_command}")
